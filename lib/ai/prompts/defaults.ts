/**
 * The seeded prompt bodies.
 *
 * These are CONTENT, not logic (functional spec §3.4): the live text is read from
 * the `prompts` table on every request, so an administrator's edit takes effect
 * immediately and applies to future analyses only. These constants are the
 * bootstrap that db/migrations/002_seed_prompts.sql inserts, and the reference a
 * test compares that migration against.
 *
...
export const NEGATIVE_MODIFIERS: Modifier[] = [
  {
    code: 'N1',
    modifier: -3,
    prompt: `Industrial trans fat from partially hydrogenated oil. This is now
...
(black, pinto, cranberry, white), peas, psyllium, farro.`,
  ...
  },
  {
    code: 'P2',
    modifier: 2,
    prompt: `The PRIMARY fat of the dish is unsaturated, and is a real component of
...
Now apply the scoring instructions that follow.`

export const SCORING_PROMPT = `# LDL IMPACT SCORING
...
export const PROMPT_KEYS = ['image_analysis_prompt', 'scoring_prompt'] as const
